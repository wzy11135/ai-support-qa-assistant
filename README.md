...
这个项目不是简单的 Chatbot，而是一个可演示、可评估、可扩展的 AI 工程项目，适合作为 AI 工程师简历项目展示。当前演示版本已接入 DeepSeek Chat API，同时保留本地 fallback 机制，保证没有 API Key 时也能完成基础演示。
...
- 知识库问答：根据售后、物流、账号和质保知识库回答客户问题。
- 引用溯源：展示 Top-K 检索片段、来源文档和相似度分数，降低无依据回答风险。
...
复制 `.env.example` 为 `.env`，填入兼容 OpenAI Chat Completions 格式的 API。`.env` 已被 `.gitignore` 排除，不会上传到 GitHub。
...
页面中显示 `模型/模式：deepseek-chat` 时，说明真实 LLM API 已成功接入。
